...
## 9. Star map — spiral galaxy (v0.3)

**Changed in v0.3** from the concentric-ring layout to a **4-arm spiral galaxy**. Canonical data: `data/map.yaml`, generated by `scripts/gen-spiral-map.mjs` (edit the generator, not the YAML). Thirty systems, structure:

- **Core (6):** Blue Silence at the galactic center plus a 5-node ring (Crown Nexus, Radiant Maw, Cryos, Solace, Orphean Vault). Densely interconnected, heavily defended (2 neutrals each), and holds 4 of the 5 Relics — the contested endgame prize.
- **Second ring (v0.4.1):** lateral lanes join the four inner-arm systems (Halcyon Deep, Aurora Gate, Greenwake, Ashen Bloom) into a ring encircling the core, closing through Orphean Vault — the one core node without an arm. Without it the core was pure hub-and-spoke: crossing between arms meant going through the center or a wormhole. Those four systems are now degree-4 junctions guarding the approach to the core, and the region around the center plays as a crossroads rather than four isolated spokes.
- **Four spiral arms (6 systems each):** each arm is a chain spiraling outward from a core node. Inner-arm systems carry 1 neutral; the outer frontier carries none.
- **Rim Gates (8):** on the outer two systems of each arm. New civilizations enter cheaply at the galactic edge and push inward toward the core.
- **Arm tips are sparse (degree 2):** a tip connects only to its spine neighbor plus one wormhole — the frontier is a defensible dead-end, not an open ring.

**Three wormholes** bridge distant points, making long-range shortcuts strategically vital in a map where crossing the galaxy otherwise means traversing an arm, the core, and another arm: Pelagos (PL)–Frostmere (FM) and Viridian Gate (VG)–Sable Rift (SR) link opposite arm tips; Altair Reach (AR)–Ossuary (OS) is a mid-arm core bypass.

**Why the spiral:** the corridor topology solved two balance problems the adversarial panel flagged on the ring map — scoring dropped from ~12.9 into the 8–13 target band (~9.1/turn, only 8% of turns exceed 13, down from 43%), and board occupancy plateaus at ~18/30 instead of saturating at 24/30 by round 7. Players push down arms and fight at chokepoints and the core rather than blob-expanding across an open board.
