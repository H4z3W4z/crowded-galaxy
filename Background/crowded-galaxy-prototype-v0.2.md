...
## 9. Star map — a procedurally generated spiral galaxy (v0.6)
...
**Terrain comes in contiguous regions (v0.7).** Each of the six types is grown as a connected blob of ~5 worlds rather than scattered, so a galaxy has an ocean sector, a volcanic frontier and so on. Each type is seeded in **two** pockets rather than one — a single seed grew each terrain into one ~5-world blob, so entire spiral arms came out a single colour and the galaxy stopped looking natural. Measured: same-terrain adjacency **42.3%** (was 14.2% — *below* the 16.7% random baseline), largest region **4.43 worlds** (was 2.89). This is what lets habitat be territory: a homeland worth taking and holding, somewhere for two players who want the same terrain to collide, and something for the generator to vary that actually changes play rather than scenery.
...
`data/map.yaml` and `scripts/gen-spiral-map.mjs` remain as a readable reference of one representative galaxy; the live game does not read them.
